...
export const getAllAlerts = async (req, res) => {
  try {
    ...
    if (userRole === 'admin' || userRole === 'directeur') {
      ...
      alerts = await AlertService.getByDestinataire(userId);
    }
...
export const getAlertsByDossier = async (req, res) => {
  try {
...
export const getAlertsByUser = async (req, res) => {
  try {
    ...
    if (userRole !== 'admin' && userRole !== 'directeur' && 
      ...
      return res.status(403).json({
        success: false,
        message: 'Forbidden: You can only view your own alerts'
      });
    }
    
    const alerts = await AlertService.getByDestinataire(parseInt(userId));
...
export const markAlertAsRead = async (req, res) => {
  try {
    ...
    if (userRole !== 'admin' && userRole !== 'directeur' && 
      ...
      return res.status(403).json({
        success: false,
        message: 'Forbidden: You can only mark your own alerts as read'
      });
    }
    
    const updatedAlert = await AlertService.markAsRead(parseInt(id));
...
export const deleteAlertsByDossier = async (req, res) => {
  try {
    const { dossierId } = req.params;
    const userRole = req.user?.role;
...
export const createDueDateReminders = async (req, res) => {
  try {
    const userRole = req.user?.role;
